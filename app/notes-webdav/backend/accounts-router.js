...
/**
 * Create an Express router for global WebDAV account management.
 *
 ...
 */
function createAccountsRouter(options = {}) {
  const {
    adapter,
  ...
  // GET /webdav-accounts — list all accounts (passwords stripped)
  ...
  router.post('/webdav-accounts', writeRateLimit, async (req, res) => {
    try {
      ...
      await adapter.saveWebdavAccounts(accounts);
      res.json({ ok: true, id });
    } catch (err) { res.status(500).json({ error: err.message }); }
  });
  ...
  // DELETE /webdav-accounts/:id — remove account
  ...
  // POST /webdav-accounts/:id/test — test connectivity for a stored account
